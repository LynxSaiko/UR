...
class LazyFrameworkGUI(QMainWindow):
    ...
    def show_splash_screen(self):
        ...
        for progress, status in loading_steps:
            ...
            time.sleep(2)  # Sedikit lebih cepat dari Burp Suite asli
        ...
        self.setStyleSheet("""
            ...
            QPushButton {
                background: qlineargradient(x1:0, y1:0, x2:0, y2:1,
                    stop:0 #3498db, stop:0.1 #2980b9, stop:0.9 #21618c, stop:1 #1b4f72);
                color: white;
    ...
    def create_new_session(self, match, raw_text):
        """Create new session from detected pattern - FIXED VERSION"""
        try:
            print(f"DEBUG: Session match groups: {match.groups()}")  # Debug
            
            if len(match.groups()) >= 3:
                # Format: Session 127.0.0.1:4444 opened (127.0.0.1:4444 -> 0.0.0.0:4444)
                sess_id = match.group(1)
                source = match.group(2)  # 127.0.0.1:4444
                destination = match.group(3)  # 0.0.0.0:4444
            ...
            # Output konfirmasi
            self.append_output(f"[bold green][+] Auto-detected: Session {sess_id}[/]")
            self.append_output(f"[dim]Switched to Sessions tab[/]")
    ...
    def update_sessions_ui(self):
        """Update sessions UI - Thread-safe version"""
        try:
            # Clear current list
            ...
            for sess_id, sess in self.sessions.items():
                item_text = f"{sess_id} | {sess.get('ip', '?.?.?.?')}:{sess.get('port', '?')} | {sess.get('type', 'unknown')}"
                ...
                item.setForeground(QColor(color_map.get(sess.get('type', ''), "#ffffff")))
                
                self.session_list.addItem(item)
                
            # Auto-select yang terbaru jika ada
            if self.session_list.count() > 0 and not self.active_session_id:
    ...
    def append_session_output(self, session_id, text):
        """Append output to specific session - Thread-safe version"""
        try:
    ...
    def on_session_selected(self, item):
        ...
        try:
            ...
            if session_id in self.sessions:
                ...
                # Update command input placeholder dengan session info
                ...
                self.append_output(f"[dim]Selected Session {session_id}[/]")
    ...
    def send_session_command(self):
        ...
        # Untuk reverse_tcp, kita kirim command via socket
        if session.get('type') == 'reverse_tcp' and session.get('socket'):
            try:
                # Handle special commands
                processed_command = self.process_special_commands(command)
                
                # Kirim command ke socket
                session['socket'].send((processed_command + "\n").encode())
                
                # Baca output setelah kirim command
                QTimer.singleShot(300, lambda: self.read_socket_output(session))
                
            except Exception as e:
                error_msg = f"Error sending command: {str(e)}\n"
                session['output'] += error_msg
                self.session_output.setPlainText(session['output'])
                self.session_output.moveCursor(QTextCursor.MoveOperation.End)
        else:
            # Jika tidak ada socket, coba dapatkan dari SESSIONS global di reverse_tcp
            self.try_get_socket_from_global(session)
            
            if session.get('socket'):
                # Coba lagi dengan socket yang baru didapat
                try:
                    processed_command = self.process_special_commands(command)
                    session['socket'].send((processed_command + "\n").encode())
                    QTimer.singleShot(300, lambda: self.read_socket_output(session))
    ...
    def try_get_socket_from_global(self, session):
        """Try to get socket from reverse_tcp's global SESSIONS"""
        try:
            # Import reverse_tcp module
            from modules.payloads.reverse import reverse_tcp
            
            session_id = session['id']
            if session_id in reverse_tcp.SESSIONS:
                global_session = reverse_tcp.SESSIONS[session_id]
                if 'socket' in global_session:
                    session['socket'] = global_session['socket']
                    self.append_output(f"[green]✓ Connected socket for session {session_id}[/]")
        except Exception as e:
            self.append_output(f"[yellow]Warning: Could not get socket from global: {e}[/]")

    def read_socket_output(self, session):
        """Read output from socket"""
        try:
            if not session.get('socket'):
                return
                
            # Set socket timeout untuk non-blocking read
            session['socket'].settimeout(0.1)
            ...
            try:
                ...
                # Timeout normal, berarti tidak ada data lagi
                pass
            except Exception as e:
                # Error lainnya
            ...
            # Process output jika ada
            if output_data:
                output_text = output_data.decode('utf-8', errors='replace')
                
                # Bersihkan output dari control characters
                import re
                clean_output = re.sub(r'\x1b\[[0-9;]*[mGKH]', '', output_text)
                clean_output = re.sub(r'\x1b\][0-9];[^\x07]*\x07', '', clean_output)  # Remove OSC sequences
                clean_output = clean_output.replace('\r\n', '\n').replace('\r', '\n')
                
                # Tambahkan ke session output
            ...
            session['output'] += f"Read error: {str(e)}\n"
            self.session_output.setPlainText(session['output'])

    def process_special_commands(self, command):
        """Process special commands before sending to target"""
        ...
        # Handle cd command - tambahkan pwd setelahnya untuk konfirmasi
        if cmd == 'cd':
            if len(parts) > 1:
                return f"cd {parts[1]} && pwd"
            else:
                return "cd && pwd"
        
        # Handle ls command dengan formatting
        elif cmd == 'ls':
            flags = ' '.join(parts[1:]) if len(parts) > 1 else '-la'
            return f"ls {flags} 2>/dev/null || dir {flags}"
        ...
        # Default: return command as-is
        return command

    def execute_special_command(self, command):
        """Execute special command from quick buttons"""
        if not self.active_session_id or self.active_session_id not in self.sessions:
            self.append_output("[red]No active session selected[/]")
            return
            
        # Set command ke input field dan trigger send
    ...
    def run_module(self):
        ...
        # Disable run button selama execution
        self.run_btn.setEnabled(False)
        self.run_btn.setText("RUNNING...")

        # Gunakan ModuleRunner
