...
class LazyFrameworkGUI(QMainWindow):
    ...
    def append_session_output(self, session_id: str, output_line: str):
        """Menambahkan baris output ke text area sesi yang sesuai."""
        
        # Pastikan Anda menemukan widget output sesi yang benar
        # (Logika ini sangat tergantung pada struktur sesi tabs Anda)
        try:
            # Asumsi: Anda memiliki cara untuk mendapatkan QPlainTextEdit dari session_id
            session_widget = self.get_session_output_widget(session_id) 
            
            if session_widget:
                # 1. Tambahkan output baru
                session_widget.appendPlainText(output_line)
                
                # 2. Pindahkan kursor ke baris paling bawah secara otomatis
                cursor = session_widget.textCursor()
                cursor.movePosition(QTextCursor.MoveOperation.End)
                session_widget.setTextCursor(cursor)
                
        except Exception as e:
            # Log error jika gagal update GUI
            self.append_log(f"[yellow]Warning: Gagal update output sesi {session_id}: {e}[/]")
    ...
    def send_session_command(self):
        ...
        if session.get('type') == 'reverse_tcp' and session.get('socket'):
            ...
            # Simulate response untuk session types lainnya
    ...
    # Di dalam gui.py, di dalam class MainWindow:

    def handle_send_command(self):
        """Fungsi yang dipanggil saat tombol 'Send' ditekan."""
        session_id = self.get_current_session_id()
        command = self.command_input_box.text().strip()
        
        if not session_id:
            self.append_log("[!] ERROR: Tidak ada sesi aktif yang dipilih.")
            return

        if not command:
            return 

        # 1. Bersihkan input box sebelum pengiriman
        self.command_input_box.clear()
        
        # 2. Kirim perintah dalam thread agar UI tidak beku
        threading.Thread(
            target=self._run_send_command,
            args=(session_id, command),
            daemon=True
        ).start()

    def _run_send_command(self, session_id, command):
        """Wrapper untuk memanggil fungsi pengirim di backend."""
        success = send_command_to_session(session_id, command)
        
        # KODE HANYA SAMPAI DI SINI. 
        # Pastikan Anda TIDAK memiliki baris seperti:
        # print(f"Command executed: {command}") 
        # ATAU
        # self.append_session_output(session_id, f"Command executed: {command}")
        
        if not success:
            # Jika gagal, perbarui UI/log
            self.append_log(f"[red]Gagal mengirim perintah ke {session_id}. Sesi mungkin ditutup.[/]")
